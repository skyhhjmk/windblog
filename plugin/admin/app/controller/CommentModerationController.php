...
class CommentModerationController extends Base
{
    ...
    public function logs(Request $request): Response
    {
        ...
        $logs = $query->with(['post', 'author'])->orderBy('created_at', 'desc')->forPage($page, $limit)->get();
        ...
        return json(['code' => 0, 'msg' => 'success', 'count' => $total, 'data' => $logs]);
    }
    ...
    public function batchRemoderate(Request $request): Response
    {
        ...
        try {
            ...
            return json(['code' => 0, 'msg' => '已入队，等待审核', 'data' => ['count' => $enqueued]]);
        } catch (Throwable $e) {
            return json(['code' => 500, 'msg' => '入队失败：' . $e->getMessage()]);
        }
    }
    ...
    public function config(Request $request): Response
    {
        if ($request->method() === 'GET') {
            ...
            $config = ['enabled' => blog_config('comment_ai_moderation_enabled', false, true), 'group_id' => $groupId, 'failure_strategy' => blog_config('comment_ai_moderation_failure_strategy', 'approve', true), 'prompt' => (function () {
                ...
                return $p;
            })(), 'auto_approve_on_pass' => (bool) blog_config('comment_ai_auto_approve_on_pass', false, true), 'auto_approve_min_confidence' => (float) blog_config('comment_ai_auto_approve_min_confidence', 0.85, true), 'temperature' => (float) blog_config('comment_ai_moderation_temperature', 0.1, true), ];
