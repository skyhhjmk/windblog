            ...
            Timer::add(1, function () {
                try {
                    if ($this->mqChannel === null) {
                        Log::warning('AI moderation: channel is null, reconnecting...');
                        $this->reconnectMq();
                    ...
                    Log::warning('AI moderation wait: ' . $errorMsg);
                    ...
                    if (strpos($errorMsg, 'Channel connection is closed') !== false ||
                        ...
                        Log::warning('AI moderation 检测到连接断开，尝试重建连接');
                        $this->reconnectMq();
                    }
                }
            });
...
EOT;
    ...
    protected function reconnectMq(): void
    {
        try {
            ...
            Log::info('AiModerationWorker MQ连接重建成功');
        } catch (Throwable $e) {
            Log::error('AiModerationWorker MQ连接重建失败: ' . $e->getMessage());
            $this->mqChannel = null;
        }
    }
}
