...
class I18nService
{
    ...
    /**
     * 获取可用语言列表
     * 从 blog_config 中读取语言配置
     *
     * @return array 语言列表，格式：[['code' => 'zh-CN', 'name' => 'Chinese', 'native_name' => '中文']]
     */
    public static function getAvailableLocales(): array
    {
        try {
            // 从 blog_config 读取语言配置
            $localesConfig = blog_config('available_locales', null, false, true);

            // 如果配置为空或格式不正确，返回默认语言列表
            if (!$localesConfig || !is_array($localesConfig)) {
                return self::getDefaultLocales();
            }
    ...
    /**
     * 获取默认语言列表
     * 使用 ISO 639-1 和 ISO 3166-1 标准的地区代码
     *
     * @return array
     */
    private static function getDefaultLocales(): array
    {
        return [
            [
                'code' => 'zh-CN',
