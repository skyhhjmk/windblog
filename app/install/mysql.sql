...
(
    50
) NOT NULL COMMENT '实体类型: post, category, tag, setting等',
    ` entity_id ` BIGINT UNSIGNED NOT NULL COMMENT '实体ID',
    ` field_name ` VARCHAR
(
    50
) NOT NULL COMMENT '字段名',
    ` locale ` VARCHAR
(
    10
) NOT NULL COMMENT '语言代码',
    ` content ` LONGTEXT NOT NULL COMMENT '翻译内容',
...
(
    ...
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE =utf8mb4_unicode_ci COMMENT ='多语言内容表';
...
(
    10
) NOT NULL UNIQUE COMMENT '语言代码',
    ` name ` VARCHAR
(
    50
) NOT NULL COMMENT '语言名称',
    ` native_name ` VARCHAR
(
    50
) NOT NULL COMMENT '本地名称',
    ` enabled ` TINYINT
(
    1
...
VALUES
    ('zh_CN', 'Simplified Chinese', '简体中文', 1, 1, 0)
ON DUPLICATE KEY
UPDATE ` enabled `=
VALUES (` enabled `);
INSERT INTO ` i18n_languages ` (` code `, ` name `, ` native_name `, ` enabled `, ` is_default `, ` sort_order `)
VALUES
    ('en', 'English', 'English', 1, 0, 1)
ON DUPLICATE KEY
UPDATE ` enabled `=
VALUES (` enabled `);
