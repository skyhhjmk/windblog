...
(
    ...
    `target`      varchar(20)                  DEFAULT '_blank' COMMENT '打开方式 (_blank, _self等)',
    `redirect_type` enum('direct','goto','iframe','info') NOT NULL DEFAULT 'info' COMMENT '跳转方式: direct=直接跳转, goto=中转页跳转, iframe=内嵌页面, info=详情页',
    `show_url`    boolean             NOT NULL DEFAULT true COMMENT '是否在中转页显示原始URL',
    `content`     longtext                     DEFAULT NULL COMMENT '链接详细介绍(Markdown格式)',
    `created_at`  datetime                     DEFAULT NULL COMMENT '创建时间',
    `updated_at`  datetime                     DEFAULT NULL COMMENT '更新时间',
    `deleted_at`  datetime                     DEFAULT NULL COMMENT '删除时间',
    PRIMARY KEY (`id`),
